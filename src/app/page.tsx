  ...
  return (
    <main className="min-h-screen p-4 sm:p-6 md:p-10">
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        {/* Hero Section */}
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 p-6 sm:p-8 md:p-12 text-white shadow-xl">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-7 shadow-xs space-y-5">
          ...
          {topicsLoading ? (
            ...
            <p className="text-slate-500 text-sm py-4">Brak zadań w bazie danych.</p>
          ) : (
            <div className="grid gap-2.5">
              {topics.slice(0, 5).map((topic) => (
                <button
                  ...
                  disabled={creating}
                  className="w-full text-left group flex items-start gap-3 rounded-xl border border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50/70 p-3.5 transition-colors cursor-pointer disabled:opacity-50"
                >
                  <span className="shrink-0 inline-flex items-center justify-center w-7 h-7 rounded-lg bg-slate-100 text-slate-800 font-mono text-xs font-bold">
                    #{topic.numer}
                  </span>
                  ...
                  <span className="text-xs font-medium text-slate-400 group-hover:text-slate-900 transition-colors self-center shrink-0">
                    Rozwiąż →
                  </span>
                </button>
              ))}
